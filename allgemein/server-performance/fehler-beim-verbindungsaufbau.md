...
<figure class="wiki-illus"><img src="/img/wiki-illus/verbindungsfehler.webp" alt="Die drei typischen Ursachen: 1 – die Verbindung bricht ab (io.netty.channel), 2 – Zeitüberschreitung (Timeout), 3 – zu viele Pakete (Packet-Spam)."><figcaption>Die drei typischen Ursachen: 1 – die Verbindung bricht ab (io.netty.channel), 2 – Zeitüberschreitung (Timeout), 3 – zu viele Pakete (Packet-Spam).</figcaption></figure>
...
* **Java veraltet**
  * Der Minecraft-Launcher kommt standardmäßig mit einer eigenen Java-Instanz, welche mit Client-Updates auch aktualisiert wird. Manchmal ist diese dennoch veraltet oder fehlerhaft. Hier hilft es dann, im Launcher unter _Installationen > Bearbeiten > Mehr Optionen_ eine aktuelle Java-Instanz vom eigenen PC zu nutzen.
  * Java kann in der Systemsteuerung deines Betriebssystems aktualisiert werden. Falls du Java noch nicht separat installiert hast, kannst du dir einen Installer von der [offiziellen Webseite herunterladen](https://www.java.com/de/download/).
* Benutzt du die **passende Minecraft-Version** für den Server?
  ...
  * Für das optimale Spielen auf GrieferGames.net "Citybuild Cloud" wird die aktuelle Minecraft-Version empfohlen!
...
* Verbindest du dich auf **GrieferGames** über <kbd>griefergames.net</kbd>?
  ...
  * Für das Verbinden auf unsere Server empfehlen wir die Adresse <kbd>griefergames.net</kbd>.
    * Die Alternative für die Java-Edition ist: <kbd>alt.griefergames.net</kbd>
    * Die Alternativadresse <kbd>cloud.griefergames.net</kbd> kann direkt auf unser [Cloud-Netzwerk](https://wiki.griefergames.live/) verbinden. Du benötigst hierfür **mindestens** den [Premium-Rang](../../funktionen/befehlsuebersicht/rang-befehle.md#premium).
...
* Spielst du über einen **VPN/Proxy/festen DNS-Server**, welcher deine Daten "umleitet"?
  * Versuche einen direkten Verbindungsaufbau ohne "Zwischenstopp", um einen Fehler bei diesem auszuschließen.
...
* Bitte überprüfe, dass deine **Firewall** nicht den Minecraft-Port `25565` und den Bedrock-Port `19132` über TCP und UDP blockiert.
  ...
  * Die Alternativports der Java-Edition sind `25566` und `25567`. Für eine optimale Freigabe sollte daher der Bereich <kbd>25565 - 25567</kbd> auf TCP und UDP freigegeben sein.
  * Der Alternativport der Bedrock-Edition ist `19134`. Für eine optimale Freigabe sollten daher die Ports <kbd>19132, 19134</kbd> auf TCP und UDP freigegeben sein.
* Wenn du die Bedrock-Edition verwendest, kann das **Ändern von Einstellungen** den Datenverkehr reduzieren und damit die Qualität verbessern (weniger Datenbelastung). Hierfür nutzt du auf unserem Server das _Menü > Einstellungen_ und den Befehl `/settings`.
...
* Für eine **stabile** Internetverbindung ist eine Bandbreite von mehr als 6 MBit/s im Downstream und 2 MBit/s im Upstream erforderlich. Wir **empfehlen** eine Bandbreite von mindestens 16 MBit/s im Downstream und 6 MBit/s im Upstream.
  * Die genaue Bandbreite des Anschlusses lässt sich über das Menü eures Routers ermitteln. In der Statusübersicht werden die Werte normalerweise angegeben.
  * Alternativ könnt ihr jederzeit eine Prüfung des Anschlusses durch euren Anbieter veranlassen.
* Die **Latenz** beschreibt die Dauer, die das Signal zwischen Client und Server und zurück benötigt. Sie wird auch als "ping" bezeichnet.
  ...
  * Die Latenz sollte in keinem Fall über 100 ms liegen.
...
* Mobile Daten werden über die Mobilfunk-Anbieter nach einem **"shared medium"-Prinzip** ausgegeben.
  * Die Verbindungsqualität schwankt daher abhängig von den aktuell an der Funkzelle verbundenen Nutzern und deren Nutzungsverhalten.
  * Teste ggf. die Verbindung über das Netzwerk des Routers.

Eine letzte mögliche Ursache liegt natürlich auch auf der Serverseite: "Lag-Spikes" des Servers.

Ist der Server gerade durch zu viele Anfragen ausgelastet _(wie bei DDoS-Angriffen/Hoster-Problemen)_, dauert es ebenfalls einen kurzen Moment, bis deine Datenpakete verarbeitet werden. Wenn diese sich in der Zeit "anstauen", kann dies ebenfalls als "Packet-Spam" interpretiert werden und du wirst vom Server geworfen.
